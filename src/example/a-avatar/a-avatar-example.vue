<template>
  <div class="a-avatar-example">
    <a-avatar src="http://www.pujia8.com/static/pics/20171215040134_26.jpg"/>
    <a-avatar src="http://www.pujia8.com/static/pics/20171215040134_26.jpg" shape="star"/>
    <a-avatar src="http://www.pujia8.com/static/pics/20171215040134_26.jpg" shape="heart"/>
  </div>
</template>

<script>
  import AAvatar from "../../base/components/a-avator/a-avatar";

  export default {
    name: "a-avatar-example",
    components: {AAvatar}
  }
</script>

<style lang="scss">

</style>
